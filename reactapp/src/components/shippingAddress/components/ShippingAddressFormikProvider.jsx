...
const initValidationSchema = {
  ...
  phone: YupString()
    ...
    .matches(
      /^(\+\d{1,3}[- ]?)?\d{9}$/, // This is a basic regex for phone numbers
      __('Please enter more or equal than 9 symbols')
    ),
  city: YupString().required(requiredMessage),
  region: YupString().required(requiredMessage),
  country: YupString().required(requiredMessage),
  phone_no_country_code: YupString().required(requiredMessage),
...
function ShippingAddressFormikProvider({ children, formikData }) {
  ...
  const { cartselectedshippingmethod, shipping_address } = cart;
  ...
  useEffect(() => {
    if (aggregatedData) {
      let shippingAddress = aggregatedData?.cart?.shipping_address || {};
      if (cartselectedshippingmethod?.carrier_code !== 'clickncollect') {
        shippingAddress = shipping_address;
      }
      const saveInBook = !!aggregatedData?.customer?.customer?.email;
      setInitialValues({ ...defaultValues, ...shippingAddress, saveInBook });
    }
  }, [aggregatedData, cartselectedshippingmethod]);
