...
export async function ajaxLoginAction(dispatch, userCredentials) {
  try {
    ...
    if (!errors) {
      ...
      const firstName = _get(data, 'customer.firstname');
      const fullName = _get(data, 'customer.fullname');
      const websiteId = _get(data, 'customer.websiteId');
      const cartId = _get(data, 'cart.cartId') || sourceCartId;

      LocalStorage.saveCartId(cartId);
      LocalStorage.saveCustomerToken(signInToken);
      LocalStorage.saveCustomerFirstName(firstName);
      LocalStorage.saveCustomerFullName(fullName);
      LocalStorage.saveCustomerWebsiteId(websiteId);
