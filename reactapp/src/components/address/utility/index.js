...
export function prepareCountryStateOptions(stateList, countrySelected) {
  ...
  const stateListObj = _get(stateList, countrySelected, []).reduce(
    (accumulator, state) => {
      if (state.code && state.name && state.id) {
        accumulator[state.code] = {
          value: state.code,
          label: state.name,
          id: state.id,
        };
...
// export function prepareCountryCityOptions(regionList) {
//   // this will make sure there will be always a unique country id; avoid duplicates if any
//   // also it will make sure it will avoid empty country entries
//   const cityListObj = regionList.reduce((accumulator, cities) => {
//     if (cities.id && cities.name) {
//       accumulator[cities.id] = { value: cities.id, label: cities.name };
//     }
//     return accumulator;
//   }, {});

//   // sorting by label
//   return _objToArray(cityListObj).sort(sortByItemLabel);
// }
